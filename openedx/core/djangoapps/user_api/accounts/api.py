...
def update_account_settings(requesting_user, update, username=None):
    ...
    if "esdoce" in update:
        try:
            esdoce_int = int(update["esdoce"])
            if esdoce_int not in range(1, 3):
                field_errors["esdoce"] = {
                    "developer_message": u"The valid values are '1' or '2'",
                    "user_message": _(u"Error in esdoce")
                }
        except ValueError:
            field_errors["esdoce"] = {
                "developer_message": u"Error types. The valid values are '1' or '2'",
                "user_message": _(u"Error in esdoce")
            }

    if "comuni" in update:
        try:
            comuni_int = int(update["comuni"])
            if comuni_int not in range(0, 20):
                field_errors["comuni"] = {
                    "developer_message": u"The valid values are from '1' to '19'",
                    "user_message": _(u"Error in comuni")
                }
            if existing_user_profile.country.code != u"ES" and comuni_int != 0:
                field_errors["comuni"] = {
                    "developer_message": u"You can't select an autonomous community if your country is not Spain",
                    "user_message": _(u"You can't select an autonomous community if your country is not Spain")
                }
        except ValueError:
            field_errors["comuni"] = {
                "developer_message": u"Error types. The valid values are '1' or '19'",
                "user_message": _(u"Error in comuni")
            }

    if "camp2" in update:
        try:
            camp2_int = int(update["camp2"])
            if camp2_int not in range(0, 4):
                field_errors["camp2"] = {
                    "developer_message": u"The valid values are from '0' to '3'",
                    "user_message": _(u"Error in camp2")
                }
        except ValueError:
            field_errors["camp2"] = {
                "developer_message": u"Error types. The valid values are '0' or '3'",
                "user_message": _(u"Error in camp2")
            }
    ...
    try:
        ...
        if existing_user_profile.country.code != u"ES":
            existing_user_profile.comuni = "0"
            existing_user_profile.save()
        elif 'comuni' in update:
            existing_user_profile.comuni = update["comuni"]
            existing_user_profile.save()

        if 'esdoce' in update:
            existing_user_profile.esdoce = update["esdoce"]
            existing_user_profile.save()

        if 'camp1' in update:
            existing_user_profile.camp1 = update["camp1"]
            existing_user_profile.save()

        if 'camp2' in update:
            existing_user_profile.camp2 = update["camp2"]
            existing_user_profile.save()

        if 'camp3' in update:
            existing_user_profile.camp3 = update["camp3"]
            existing_user_profile.save()

        if 'camp4' in update:
            existing_user_profile.camp4 = update["camp4"]
            existing_user_profile.save()

        if 'camp5' in update:
            existing_user_profile.camp5 = update["camp5"]
            existing_user_profile.save()
