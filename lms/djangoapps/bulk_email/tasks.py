...
def _get_course_email_context(course):
    ...
    email_context = {
        ...
        'account_settings_url': 'http://{}{}'.format(settings.SITE_NAME, reverse('account_settings')),
        'email_settings_url': 'http://{}{}'.format(settings.SITE_NAME, reverse('dashboard')),
...
def perform_delegate_email_batches(entry_id, course_id, task_input, action_name):
    ...
    recipient_qsets = _get_recipient_querysets(user_id, to_option, course_id, email_obj.custom_to_addr)
...
def _send_course_email(entry_id, email_id, to_list, global_email_context, subtask_status):
    ...
    if course_email_template.name is not None and 'eneral' in course_email_template.name:
        subject = "[MOOC INTEF] " + course_email.subject
    else:
...
def split_emails_or_throw_error(str_addresses):
    """
    Split emails by comma, if there's an email malformed it throws an error
    Args:
        str_addresses: string of emails concatenated by ','

    Returns:
        list of string with the email addresses
    """
    str_addresses = str_addresses.replace('\n','').replace(' ','')
    addresses = [addr.strip() for addr in str_addresses.split(",")]
