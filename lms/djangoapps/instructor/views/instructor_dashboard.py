...
def instructor_dashboard_2(request, course_id):
    ...
    if bulk_email_is_enabled_for_course(course_key):
        ...
        all_templates = CourseEmailTemplate.objects.filter(microsite_target=microsite.get_value('domain_prefix', 'mooc'), type_of_email='course_email')
        email_template_name = [(email_template.name, email_template.default_selected) for email_template in all_templates]
        context['option_templates'] = email_template_name
...
def _section_student_admin(course, access):
    ...
    section_data = {
        ...
        'last_access_url': reverse('last_access', kwargs={'course_id': unicode(course_key)}),
        'get_csv_last_access_url': reverse('get_csv_last_access', kwargs={'course_id': unicode(course_key)})
    }
    return section_data
...
def _section_send_email(course, access):
    ...
    section_data = {
        ...
        'access': access,
        'get_emails_from_csv': reverse('get_emails_from_csv', kwargs={'course_id': unicode(course_key)}),
