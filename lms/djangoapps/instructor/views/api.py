...
from bulk_email.tasks import split_emails_or_throw_error
...
@require_level('staff')
def get_emails_from_csv(request, course_id):
    emails_response = {'emails':[]}
    try:
        file_to_parse = request.FILES.popitem()[1]
        emails = _extract_emails_from_csv(file_to_parse)
        emails_response['emails'] = emails
    except Exception as e:
        emails_response['error'] = _("The CSV file is no correct.")

    return JsonResponse(emails_response)

def _extract_emails_from_csv(file):
    emails = []
    csv_data = csv.reader(file[0])
    for row in csv_data:
        try:
            validate_email(row[0])
            emails.append(row[0])
        except:
            print("CSV value '%s' not included in the email result." % (row[0]))
...
@cache_control(no_cache=True, no_store=True, must_revalidate=True)
@require_level('staff')
def get_csv_last_access(request, course_id):  # pylint: disable=unused-argument
    """
    Respond with 3-column CSV output of email, name and last_access
    """
    course_id = SlashSeparatedCourseKey.from_deprecated_string(course_id)
    ...
    header = ['Email', 'Name', 'Last Access']
    rows = []
    for s in enrollments:
        single_enrollment = []
        if s.last_access is not None:
            date = s.last_access.astimezone(timezone.get_current_timezone()).strftime("%d/%m/%Y %H:%M")
        else:
            date = "--"
        single_enrollment.append(s.user.email)
        single_enrollment.append(s.user.profile.name)
        single_enrollment.append(date)
        rows.append(single_enrollment)
    return csv_response(course_id.to_deprecated_string().replace('/', '-') + '-list-last-user-access.csv', header, rows)
...
def send_email(request, course_id):
    ...
    custom_to_addr = request.POST.get("custom_to_addr")

    addresses_validation = []
    try:
        if send_to == SEND_TO_CUSTOM:
            addresses_validation = _validate_emails(course_id, custom_to_addr)
    except:
        response_payload = {
            'course_id': course_id.to_deprecated_string(),
            'success': False
...
def _validate_emails(course_id, custom_to_addr):
    """
    Validate email: if is in Opt-out or if it is not registered in the platform
    Args:
        course_id:course id
        custom_to_addr: list of addresses

    Returns:
        list of messages if there's any problem with an address
    """
    messages = []
    addresses = split_emails_or_throw_error(custom_to_addr)

    # 1. If any address is in Opt-out (message)
    opt_out_users = Optout.objects.filter(
        course_id=course_id,
        user__email__in=addresses
    )

    if opt_out_users:
        messages.append(_('Next emails has been excluded on request of its own user: {emails}. ').format(emails=", ".join([user.email for user in opt_out_users])))

    # 2. If any address is not in users
    users_found = User.objects.filter(email__in=addresses)
    emails_found = [user.email for user in users_found]
    emails_not_found = [email for email in addresses if email not in emails_found]

    if emails_not_found:
        messages.append(_('Next emails has been excluded because it has no user account associated: {emails}. ').format(emails=", ".join(emails_not_found)))
