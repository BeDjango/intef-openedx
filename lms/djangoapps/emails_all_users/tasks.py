...
class BaseTask(Task):
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        try:
            ...
            # trying to update it.
            TASK_LOG.error(u"Task (%s) has no EmailAllUsersTask object for id %s", task_id, entry_id)
        else:
            TASK_LOG.warning(u"Task (%s) failed", task_id, exc_info=True)
            entry.task_output = EmailAllUsersTask.create_output_for_failure(einfo.exception, einfo.traceback)
