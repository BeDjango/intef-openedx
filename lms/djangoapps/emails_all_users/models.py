...
class EmailAllUsersTask(models.Model):
    """
    Stores information about background tasks that have been submitted to
    perform work for email for all users.
    ...
    def create(cls, task_type, task_key, task_input, requester):
        ...
        if len(json_task_input) > 255:
            fmt = 'Task input longer than 255: "{input}" for "{task}"'
            msg = fmt.format(input=json_task_input, task=task_type)
            raise ValueError(msg)

        # create the task, then save it:
        all_users_email_task = cls(
            task_type=task_type,
            task_id=task_id,
            task_key=task_key,
        ...
        all_users_email_task.save_now()

        return all_users_email_task
