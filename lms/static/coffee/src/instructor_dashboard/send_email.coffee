...
class SendEmail
  constructor: (@$container) ->
    ...
    @$select_template = @$container.find("select[name='select_template']'")
    @$subject = @$container.find("input[name='subject']'")
    @$btn_send = @$container.find("input[name='send']'")
    @$file_csv = @$container.find("#id_custom_csv")
    @$custom_to_addr = @$container.find("textarea[name='custom_to_addr']'")
    ...
    # When the Send to is CUSTOM then show the textarea, otherwhise hide it
    @$custom_to_addr.parent().hide()
    @$send_to.change =>
      if @$send_to.find("option:selected")[0].value == "custom"
        @$custom_to_addr.parent().show()
      else
        @$custom_to_addr.parent().hide()

    # When file is changed: alter te textarea to write the emails
    @$file_csv.change =>
      if not @$file_csv[0].files or @$file_csv[0].files.length
          # Prepares a FormData with the file included and send to the API query for get emails
          send_data = new FormData()

          $.each(@$file_csv[0].files, (i, file) => send_data.append('file-' + i, file))
          ...
          $.ajax
            ...
            contentType: false,
            processData: false,
            url: @$file_csv.data 'endpoint'
            data: send_data
            success: (data) =>
              @$custom_to_addr.val(data.emails)
    ...
    @$btn_send.click =>
      if @$subject.val() == ""
      ...
      else if @$send_to.find("option:selected")[0].value == "custom" and (!@$custom_to_addr.val() or @$custom_to_addr.val().trim() == "")
        alert gettext("Your message cannot be blank.")
      else
        ...
        if confirm full_confirm_message
          ...
          send_data =
            ...
            send_to: @$send_to.val()
            subject: @$subject.val()
            custom_to_addr: @$custom_to_addr.val()
            select_template: @$select_template.val()
          ...
          $.ajax
            ...
            data: send_data
            success: (data) =>
              if data.success and data.addresses_validation
                @display_response (success_message + '\n' + data.addresses_validation.join(", "))
